...
export function renderReviewView() {
    if (!state.currentUser) {
        render(html`<p class="text-center text-gray-500 p-8">Por favor, faça login para ver suas revisões.</p>`, DOM.reviewTableContainer);
        return;
    }

    if (state.userReviewItemsMap.size === 0) {
        render(html`<p class="text-center text-gray-500 p-8">Você ainda não tem nenhuma questão para revisar. Resolva questões para começar.</p>`, DOM.reviewTableContainer);
        return;
    }
    ...
    if (hierarchy.size === 0) {
        render(html`<p class="text-center text-gray-500 p-8">Nenhuma matéria com questões para revisar.</p>`, DOM.reviewTableContainer);
        return;
    }
    ...
    const renderRow = (node, name, level, parentId = '', pathData = {}) => {
        ...
        const dataAttributes = { ...pathData, 'data-level': level };

        let childRows = [];
        if (hasChildren) {
            const sortedChildren = Array.from(node.children.keys()).sort(naturalSort);
            childRows = sortedChildren.map(childName => {
                ...
                return renderRow(childNode, childName, level + 1, rowId, childPathData);
            });
        }

        return html`
            <tr class="${['materia-row', 'assunto-row', 'subassunto-row', 'subsubassunto-row'][level - 1]} ${level > 1 ? 'hidden' : ''} ${isDisabled ? 'bg-gray-50 text-gray-400' : 'hover:bg-gray-50'}" data-id="${rowId}" data-parent-id="${parentId}" data-level="${level}">
                <td class="px-4 py-4 whitespace-nowrap">
                    <div class="flex items-center pl-4">
                        <input type="checkbox" class="review-checkbox rounded" ?disabled=${isDisabled} .dataset=${dataAttributes}>
                    </div>
                </td>
                <td class="px-4 py-4 whitespace-nowrap font-medium ${isDisabled ? '' : 'text-gray-900'}">
                    <div class="flex items-center pl-${(level - 1) * 4}">
                        ${hasChildren
                            ? html`<i class="fas fa-chevron-right toggle-review-row transition-transform duration-200 mr-2 text-gray-400 cursor-pointer"></i>`
    ...
    const tableRows = sortedMaterias.map(materiaName => {
        const materiaNode = hierarchy.get(materiaName);
        return renderRow(materiaNode, materiaName, 1, '', { materia: materiaName });
    });
    ...
    render(tableTemplate, DOM.reviewTableContainer);
}
