...
async function renderDesempenhoMateriaTable(filters = null) {
    ...
    for (const materiaName of sortedMaterias) {
        ...
        if (hasAssuntos) {
            const sortedAssuntos = Array.from(materiaNode.assuntos.keys()).sort(naturalSort);
            for (const assuntoName of sortedAssuntos) {
                ...
                if (hasSubAssuntos) {
                    const sortedSubAssuntos = Array.from(assuntoNode.subAssuntos.keys()).sort(naturalSort);
                    for (const subAssuntoName of sortedSubAssuntos) {
                        ...
                        if (hasSubSubAssuntos) {
                            const sortedSubSubAssuntos = Array.from(subAssuntoNode.subSubAssuntos.keys()).sort(naturalSort);
